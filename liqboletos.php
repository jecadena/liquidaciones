    ...
    <div id="layoutSidenav">
            ...
            <nav class="sb-sidenav accordion sb-sidenav-dark" id="sidenavAccordion">
                <div class="sb-sidenav-menu">
                    <div class="nav">
                        ...
                        <a class="nav-link active" href="liqboletos.php">
        ...
        <main>
            ...
            <div class="container">
                <div class="row mt-4">
                    <div class="col-lg-12">
                        <h2 class="text-center mb-4">Liquidaciones por Boleto</h2>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-6">
                ...
                <div class="row bg-light mt-4 p-3">
                    ...
                    <div class="col-lg-6">
                        <div class="form-group row">
                            <label for="tipo_boleto" class="col-sm-4 col-form-label btn-sm">Tipo de boleto:</label>
                            <div class="col-sm-8">
                                <select class="form-control btn-sm" id="tipo_boleto" name="tipo_boleto">
                                    <option value="">[- Seleccione -]</option>
                                    <?php foreach ($tipos_maestro as $tipom): ?>
                                        <option value="<?php echo $tipom['co_tip_maestro']; ?>"><?php echo $tipom['de_tip_maestro']; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-6">
                        <div class="form-group row">
                            <label for="codigo_referencia" class="col-sm-4 col-form-label btn-sm">Código de Referencia:</label>
                            <div class="col-sm-8">
                                <select class="form-control btn-sm" id="codigo_referencia" name="codigo_referencia">
                                </select>
                            </div>
                        </div>
                    </div>
    ...
    <script>
       $(document).ready(function(){
            ...
            $('#tipo_boleto').change(function(){
                var tipo_boleto = $(this).val();
                $.ajax({
                    url: 'buscar_referencias.php',
                    type: 'GET',
                    data: { tipo_boleto: tipo_boleto },
                    success: function(response) {
                        var referencias = JSON.parse(response);
                        var $codigo_referencia = $('#codigo_referencia');
                        $codigo_referencia.empty();
                        
                        $codigo_referencia.append('<option value="">[- Seleccione Referencia -]</option>');
                        
                        $.each(referencias, function(index, referencia) {
                            $codigo_referencia.append('<option value="' + referencia.co_maestro + '">' + referencia.de_maestro + '</option>');
                        });
                    }
                });
            });
            ...
            $('#buscar-form').submit(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        tabla += '<thead><tr><th>Boleto</th><th>Tipo Doc</th><th>Serie</th><th>Número</th><th>File</th><th>Fecha</th><th>Monto</th></tr></thead><tbody>';
                        var sumatotal = 0;
                        $.each(documentos, function(index, documento) {
                            var monto = parseFloat(documento.mo_total);
                            sumatotal += monto;
                            tabla += '<tr>';
                            tabla += '<td><span class="badge bg-primary">' + documento.de_boleto + '</span></td>';
                    ...
                    success: function(response) {
                        ...
                        try {
                            ...
                            if (data.error) {
                                ...
                                $('#resultados').html('<div class="alert alert-warning" role="alert">Respuesta inesperada del servidor</div>');
                            }
                        } catch (e) {
                            $('#resultados').html('<div class="alert alert-danger" role="alert">Error al procesar la respuesta del servidor: ' + e.message + '</div>');
                        }
                    },
                    error: function(xhr, status, error) {
                        $('#resultados').html('<div class="alert alert-danger" role="alert">Error en la petición AJAX: ' + error + '</div>');
                    }
                });
            });
        });
